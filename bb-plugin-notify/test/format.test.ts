...
test("notification URLs use BB's project and projectless thread routes", () => {
  assert.equal(
    notificationUrl("proj_alpha", "thr_1"),
    "/projects/proj_alpha/threads/thr_1",
  );
  assert.equal(notificationUrl("proj_personal", "thr_1"), "/threads/thr_1");
  assert.equal(notificationUrl(null, "thr_1"), "/threads/thr_1");
  assert.equal(notificationUrl("proj_alpha", null), "/");
});
