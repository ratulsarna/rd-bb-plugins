...
The user tiers the task when handing it over. The lead states the tier in one line and does not re-tier.
...
Bugs get tiered after the RCA; you cannot size what you have not diagnosed. Smaller tiers drop the user touchpoints, not the gates: the short ticket and the draft PR carry the record.
