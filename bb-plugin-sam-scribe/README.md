...
in `~/assistants/sam/memory`; it keeps Ratul's daily note in the vault up to
date from Sam's conversations. That folder's README has the whole memory
design.
...
The script exits 2 when no local model is loaded. That is logged as a
